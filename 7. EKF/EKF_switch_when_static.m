clear; clc
data = load('forward_rotate_IMU.txt');
...
Xpost_pi = load('forward_rotate_x_post.txt');
figure
plot(Xpost_pi)
hold on
plot(X_post)
...
sync_data = [];
output = [Xpost_pi'; magnet_on; location(:,2)'];
...
magnet_on_padd = sync_data(2,:);
Y_post_padd = sync_data(3,:);
idx_mag_switch_padd = find( (magnet_on_padd(1:end-1)-magnet_on_padd(2:end)) ~= 0); 
save('padded_Xpost', 'X_post_padd', 'Y_post_padd', 'idx_mag_switch_padd')