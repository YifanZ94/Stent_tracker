...
%% magnetic correction
magnet_on = [0;data(init_range+1:end-1,10)];

start_ind = find(magnet_on==1,1);
mag_disturb = magnetic(start_ind-1,:);
end_ind = size(magnetic,1);

% size(magnetic,1)
for i = start_ind: end_ind
    if magnet_on(i) == 0 && magnet_on(i-1) == 1
        init_mag_disturb = magnetic(i+2,:);
        magnetic(i,:) =  mag_ref;
        
    elseif magnet_on(i) == 0
            mag_disturb = magnetic(i,:);
            distb_var = mag_disturb - init_mag_disturb;
            magnetic(i,:) =  mag_ref + distb_var;
            
    elseif magnet_on(i) == 1
        magnetic(i,:) = magnetic(i,:) - mag_disturb;
        mag_ref = magnetic(i,:);
    end 
...
%%
mag_min_thres = 0.1;
k2 = find(mag_norm<mag_min_thres);
magnetic(k2,:) = [];
magnet_on(k2,:) = [];
mag_norm(k2) = [];
mag_on_index = find(magnet_on == 1); 
mag_off_index = find(magnet_on == 0); 
times = (1:size(magnetic,1))*Ts;
%%
...
I(1) = knnsearch([BxR,ByR,BzR], mag_normlize(magnetic(1,:)), 'K', 1);
location = LocMap(I(1),:); 
...
for i = 2:L
    acc_std_static(i) = acc_amp_std(i) < a_std_0;
    
    if acc_std_static(i) == 0
...
for i = 2:L
    % NN implementation
    NN_inputs = [acc_correct(dim,i); velocity(:,i); disp_delta(:,i); acc_std_static(i); gyro_amp(i)]./sigmaX;
...
% && magnet_on(i) == 1
    ...
    if  magnet_on(i) ~=  magnet_on(i-1)
        idx_mag_switch = [idx_mag_switch, i];
    end
    ...
    if magnet_on(i) == 1
        ...
        varR = std(location_mag_off);
        if size(location_mag_off,1) == 1
            covR(i) = covR(i-1);
        else
            covR(i) = varR(dim);
        end
    ...
    innovation(:,i) = Kk(i)*(Z(i) - X_pri(1,i));
    X_post(:,i) = X_pri(:,i) + innovation(:,i);

    P_post(i) = (eye(size(Kk,1))- Kk(i)*H)*P_pri(i);        
...
legend('magnetic','inertial','fusion')
...
%%
% X_post = [X_post(1)*ones(1,start_ind), X_post];
save('Test1_KF_ori.mat', 'X_post','location','X_pri')


%%
figure
% scatter(idx_mag_switch-mag_start_ind, location(idx_mag_switch-mag_start_ind,1))

plot(magnetic_ori(start_ind:end,1) )
hold on
