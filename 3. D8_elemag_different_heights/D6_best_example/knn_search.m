...
%% KNN
% size(test,1)
Euler_angles = [45,0,0]*pi/180;

for j = 1:size(test,1)
    % measure = [test(j,1)*cos(theta)-test(j,2)*sin(theta), test(j,1)*sin(theta)+test(j,2)*cos(theta), test(j,3)];

    measure = (Rot_by_Eulers(Euler_angles(1),Euler_angles(2),Euler_angles(3))*test(j,:)')';
...
error_center = [location(center_idx,1), location(center_idx,2), location(center_idx,3)+18];
