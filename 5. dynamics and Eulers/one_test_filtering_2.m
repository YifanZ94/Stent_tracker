...
% data = data(570:1195,:);   % for servo

% data = load('6.83s_manual_IMU.txt');
% trim_range = 1:502;

data = data(trim_range,:);
...
acc_init_std = std(data(1:20, 1:3)-acc_init, 1);
gyro_init_std = std(data(1:20, 4:6), 1);
...
for i = 1:L
    acc_amp(i) = sqrt(abs(norm(acc(:,i))^2 - acc_mean^2));
...
a_std_0 = 1.01*max(acc_std_ave(1:100));
w_std_0 = 0.01;

figure
plot([zeros(1,H), acc_std_ave])
xlabel('Time (S)')
ylabel('Acc std')
figure
plot([zeros(1,H), gyro_std_ave])
xlabel('Time (S)')
ylabel('Gyro std')
...
for i = 1:L-H
    acc_is_static(i+H) = acc_std_ave(i) < a_std_0;
    gyro_is_static(i+H) = gyro_std_ave(i) < w_std_0;
end
figure
subplot(121)
...
title('acc is static')
subplot(122)
plot(gyro_is_static,'x')
title('gyro is static')
