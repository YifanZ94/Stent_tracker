...
XTest = (cat_test_input(training_dim,:) - muX) ./ sigmaX;
TTest = (cat_test_target(targer_dim,:) - muT) ./ sigmaT;

Y = predict(net,XTest,'MiniBatchSize',1);

D1 = denormalize(Y,sigmaT,muT);
D2 = cat_test_target(targer_dim,:);
...
disp('done')
save('Trained_NN_Gru_slow.mat', 'net', 'muT','muX','sigmaT','sigmaX')
